...
BERT is an example of a 'encoder-only' model, because all its attention layers allow for communication between all tokens in the input sequence. That's because its downstream applications are e.g. text classification, and sentiment analysis where a prediction is made based on all tokens at once. For this reason it can be said that it's designed for natural language understanding tasks that involve 'context-based understanding'.

e.g. In SQuAD, the model is trained to predict the start and end positions of the answer within the context of a given passage. During fine-tuning, the model is presented with a question and a passage, and it must identify the span of text within the passage that answers the question. This task is formulated as a token classification problem, where each token in the passage is assigned a label indicating whether it is the start or end of the answer span.
...
### From text generation to chat-bot

The kind of training we've described where you:

- take a large corpus of raw text you want your transformer net to model
- tokenise it
- train the model in an unsupervised way, where the predicted token from a `T` length context is compared with the known next token to compute the loss.

is called the 'pretraining' phase. The text generated by this model is not that helpful in a question-answer context though, as the model has only been trained to generate _likely_ follow-on text - not necessarily _helpful_.

The model used for ChatGPT has been 'aligned' by a process called 'fine-tuning':

- A smaller corpus of domain specific training data is collected. This contains prompts followed by known good quality answers. The pretrained model is further trained on this.
- An additional stage of reinforcement learning is done to pick responses that humans like. A policy is trained to score models, providing a loss function for further fine-tuning.
